...
We use the same template syntax for `environment` as we do for [`command`](./commands#run).
...
`run`: The command to run a linter. This command can use variables provided at runtime such as `$plugin}` and `$target}`. [Full list of variables](./commands#template-variables). See [Run](./commands#how-code-quality-runs-linters) for more details.
