...
Branch scope uses the same glob syntax as [failure rate monitor branch patterns](/flaky-tests/detection/threshold-monitor#branch-pattern-syntax) and accepts up to 10 patterns. Type a pattern and press **Enter** or comma to add it as a chip. For example:
...
You can temporarily mute the pass-on-retry monitor for a specific test case. See [Muting monitors](/flaky-tests/detection/..#muting-monitors) for details.
