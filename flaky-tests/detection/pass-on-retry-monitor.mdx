...
<Frame caption="A fail then pass on the same commit flags the test flaky; 7 clean days then resolve it to healthy.">
<img src="/assets/flaky-tests/detection/pass-on-retry-monitor.svg" alt="A test fails on attempt 1 and passes on retry on the same commit, gets flagged flaky, then passes on each of the next 7 days and resolves to healthy." />
</Frame>
...
Branch scope uses the same glob syntax as [failure rate monitor branch patterns](./failure-rate-monitor#branch-scope) and accepts up to 10 patterns. Type a pattern and press **Enter** or comma to add it as a chip. For example:
