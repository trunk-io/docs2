...
og:title: "How Trunk Flaky Tests detects flaky tests"
---
...
These deactivations are recorded in the test's detection history. On the test detail page, the **Events** tab shows a **Monitor disabled** or **Monitor deleted** card for each affected test, alongside the monitor's name and the timestamp of the change. The monitors list column that previously read **Last detection** now reads **Last event** to reflect that it tracks both detections and deactivation events.
