...
|---|---|---|---|---|
| [**Pass-on-Retry**](./pass-on-retry-monitor) | A test fails then passes on the same commit (retry after failure) | Flaky | Team and above | Enabled |
| [**Failure Rate**](./failure-rate-monitor) | Failure rate exceeds a configured percentage over a time window | Flaky or Broken | Paid plans | Disabled |
| [**Failure Count**](./failure-count-monitor) | A test accumulates a configured number of failures in a rolling window | Flaky or Broken | Paid plans | Disabled |
