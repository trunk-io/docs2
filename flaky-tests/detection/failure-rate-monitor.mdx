...
<Frame caption="As failing runs accumulate, the failure rate crosses the activation threshold (flagged flaky); as it falls, it drops below the resolution threshold (back to healthy).">
<img src="/assets/flaky-tests/detection/failure-rate-monitor.svg" alt="Test runs resolve pass or fail while a failure-rate meter rises past the 30% activation line and is flagged flaky, then falls below the 15% resolution line and resolves to healthy." />
</Frame>
