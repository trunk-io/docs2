...
<Frame>
<img className="block dark:hidden" src="/assets/github-comment-light.png" alt="" />
<img className="hidden dark:block" src="/assets/github-comment-dark.png" alt="" />
</Frame>
...
<Frame>
<img className="block dark:hidden" src="/assets/pr-test-summary.png" alt="" />
<img className="hidden dark:block" src="/assets/pr-test-summary_(1).png" alt="" />
</Frame>
...
5. The resource owner should be the GitHub Organization or user that owns the appropriate repositories. [See note on GitHub Org Ownership settings.](./github-pull-request-comments#github-org-ownership)
...
8.  If everything looks good, scroll down to double check that your Overview for permissions looks something like the image below. If so, create the token.
    ...
    <Frame>
    ![](/assets/Screenshot_2024-06-12_at_9.52.28_AM.png)
    </Frame>
9. Once the token is generated, go back to the Trunk App ([app.trunk.io](https://app.trunk.io/login?intent=flaky%20tests)), click your profile, then navigate to **Settings** → **Manage** (under _Organization_) → **Organization GitHub Token** and enter the copied token into the text field, then finally press **Submit**.
...
Pull Request comments are enabled by default. If you wish to disable the comments, you can do so by navigating to the Trunk App, clicking your profile, then **Settings** → **[RepoName]**. In the **Flaky Tests** heading, toggle the **Summary Flaky Tests Reports** setting.

## Troubleshooting

**GitHub Organization ownership.** If you wish to set the resource owner to be a GitHub Organization, you should double check that this is allowed by navigating to your **GitHub Organization** → **Settings** → **Personal access tokens** → **Settings**. Make sure under "_Fine-grained personal access tokens_", you have _"Allow access via fine-grained personal access tokens"_ selected.

Once the token is created, the Organization admin may need to approve the request for the token. This can be done by navigating to **GitHub Organization** → **Settings** → **Personal access tokens** → **Pending requests**. To confirm that the token was set, you should be able to see it under **Active tokens**.
