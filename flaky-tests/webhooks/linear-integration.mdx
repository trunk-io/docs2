...
Trunk also has a [built-in Linear integration](./linear-integration) for ticket creation. You only need to use webhooks if you want to automate ticket creation or need additional customization.
...
3.  Under **Organization > Webhooks**, click **Automate Linear Issues Creation.**
    ...
    <Frame>
    <img className="block dark:hidden" src="/assets/example-webhook-connector-light.png" alt="" />
    <img className="hidden dark:block" src="/assets/example-webhook-connector-dark.png" alt="" />
    </Frame>
...
Replace `<YOUR_API_TOKEN>` with the Linear personal access token you created in [step 1](#id-1.-create-a-linear-personal-access-token).
...
You can modify your issue create payload like this to include an assignee. Replace `<YOUR_TEAM_ID>`, `<YOUR_PROJECT_ID>`, `<YOUR_LABEL_ID>`, and `<ASSIGNEE_ID>` with the IDs you queried for above.
...
<Frame>
![](/assets/example-webhook-delivery-status.png)
</Frame>
...
<Frame>
![](/assets/example-webhook-logs.png)
</Frame>
