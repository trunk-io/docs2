...
Trunk also has a [built-in Jira integration](/flaky-tests/ticketing-integrations/jira-integration) for manual ticket creation. You only need to use webhooks if you want to automate ticket creation or need additional customization.
...
| `PRS_IMPACTED_THRESHOLD` | Issues will be created only for flaky tests that have impacted more PRs than the `PRS_IMPACTED_THRESHOLD`.<br/><br/>You can adjust this value if you see many issues about low-impact flaky tests. |
