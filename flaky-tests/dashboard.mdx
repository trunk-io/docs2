...
{/* TODO(shot): the collections list, several collections at different Setup counts, with real sparkline data. Light/dark pair under /assets/flaky-tests/. */}
...
While you are migrating, the repository view stays available behind **Legacy view** on this page. It carries the same columns per repository, plus a quarantine status icon beside each name:
