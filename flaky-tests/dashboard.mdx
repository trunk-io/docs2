...
The **Monitors** tab opens on a monitor history swimlane: one row per monitor, each showing a 30-day timeline of the classifications it produced. Bars are colored by status (or by the label's color for label monitors), and the **Show inactive and disabled monitors** toggle filters the view down to monitors with recent activity.
