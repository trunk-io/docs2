...
<Frame>
![](/assets/enable_test_quarantining.png)
</Frame>
...
<Frame caption="The row actions menu in the Flaky Tests table, with the Quarantine test and Never Quarantine test options.">
  <img className="block dark:hidden" src="/assets/qurantine-individual-tests-light.png" alt="overriding individual tests" />
  <img className="hidden dark:block" src="/assets/qurantine-individual-tests-dark.png" alt="overriding individual tests" />
</Frame>
...
Access to these four options is controlled by the [Manual Quarantine Permissions](#quarantining-settings) setting. When set to **Admins only**, non-admin users see them as disabled with the tooltip "Only admins can set manual quarantine."

**From the test details page**

* To set an override: open the quarantine status control to reveal **Choose quarantine status**, select **Always** (quarantine this test's failures) or **Never** (prevent it from being quarantined), add the required comment, and click **Save**.
* To remove an override: open the control again and click **Remove Quarantine**.
...
<Frame>
![](/assets/qurantine-audit-logs.png)
</Frame>
