...
og:title: "Quarantining flaky tests with Trunk"
---
<Frame>
![](/assets/flaky-tests/quarantining/quarantined_test1.png)
</Frame>
...
<Frame caption="The same failing test blocks the PR without quarantine (exit code 1); quarantined, its failure is ignored and the PR merges (exit code 0).">
<img src="/assets/flaky-tests/quarantining/quarantining.svg" alt="A CI job runs three tests where test_checkout fails. Without quarantine the job exits 1 and the PR is blocked; with the failing test wrapped in a quarantine box the job exits 0 and the PR merges." />
</Frame>
...
Actively quarantining tests will significantly change CI results, as failures from quarantined tests no longer cause builds to fail. [Learn more about the effects of quarantining](#what-does-quarantined-mean).
</Note>
...
Quarantining is enabled by default on a [test collection](../test-collections). To change it, navigate to **Flaky Tests** → **Collections** → **[collection]** → **Settings** → **Quarantining**.
...
Manual **Always Quarantine** and **Never Quarantine** overrides can only be set once quarantining is enabled on the collection. With it off, the override control on a test's details page is disabled and points you at these settings — so a collection's overrides are always re-created after quarantining is turned on, never staged before it.
...
<Frame caption="The Choose quarantine status control on a test's details page, set to Always.">
  <img className="block dark:hidden" src="/assets/flaky-tests/quarantining/qurantine-individual-tests-light.png" alt="quarantine override on the test details page" />
  <img className="hidden dark:block" src="/assets/flaky-tests/quarantining/qurantine-individual-tests-dark.png" alt="quarantine override on the test details page" />
</Frame>
...
<Frame>
![](/assets/flaky-tests/quarantining/qurantine-individual-tests-revert.png)
</Frame>
...
<Frame>
![](/assets/flaky-tests/quarantining/flaky-fullscreen.png)
</Frame>
