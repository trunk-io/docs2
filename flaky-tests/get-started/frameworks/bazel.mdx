...
<Note>
  Trunk accepts BEP files in both JSON and binary formats. Use `--build_event_json_file` for JSON or `--build_event_binary_file` for the binary protobuf encoding — pass either file to the analytics CLI's `--bazel-bep-path`. The CLI detects the format automatically, so you don't need to tell it which one you used.
</Note>
