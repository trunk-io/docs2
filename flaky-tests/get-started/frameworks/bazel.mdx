...
<Warning>
  Do not use `--test_keep_going=false` (or `--notest_keep_going`) when running tests you intend to quarantine. With keep-going disabled, Bazel stops at the first test failure, so subsequent tests never run and never appear in the BEP report. Trunk can only quarantine failures it sees, so any tests that were skipped due to an earlier failure will still break your CI even if they're quarantined. Leave `--test_keep_going` at its default (`true`) so every test runs and reports a result.
</Warning>
