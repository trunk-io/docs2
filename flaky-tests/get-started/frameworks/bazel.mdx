...
<Warning>
  Keep `--test_keep_going` at its default (`true`) when you quarantine tests. With `--test_keep_going=false` (or `--notest_keep_going`), Bazel aborts on the first failing test, so later tests never run and never reach the BEP report. If that first failure is a quarantined test, Trunk sees only an already-quarantined failure, marks the run as passing, and lets the PR merge while the tests that never ran might have failed.
</Warning>
