...
<Note>
  Leave `--test_keep_going` at its default (`true`) when you intend to quarantine tests. With `--test_keep_going=false` (or `--notest_keep_going`), Bazel stops at the first test failure, so subsequent tests never run and never appear in the BEP report. Trunk can only quarantine failures it sees, so any test skipped because of an earlier failure will still break your CI even if it's quarantined.
</Note>
