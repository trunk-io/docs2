...
{/* TODO(Tyler): screenshot of the migrate page. Shoot the collection's Monitors tab, Migrate from a repository,
    on a MIGRATION_IN_PROGRESS org, with a repository picked that yields a row in all three groups and the
    "Settings differ" diff expanded. Save as assets/flaky-tests/get-started/migrate-monitors-light.png / -dark.png,
    then uncomment the Frame below and write the alt text. */}
{/*
<Frame caption="Every monitor on the repository, sorted by whether the collection already has it.">
  <img className="block dark:hidden" src="/assets/flaky-tests/get-started/migrate-monitors-light.png" alt="" />
  <img className="hidden dark:block" src="/assets/flaky-tests/get-started/migrate-monitors-dark.png" alt="" />
</Frame>
*/}
...
A new collection starts with quarantining enabled, and it replaces the repository's quarantining for every upload routed there. Your repository's **Always Quarantine** and **Never Quarantine** overrides do not follow, so re-apply the ones that matter.
</Step>
...
<Warning>
A collection you create now has quarantining enabled from the start, with no overrides in place. Between creating it and re-applying your overrides, every upload routed there is governed by the collection's settings and nothing else — a test you had pinned to **Never Quarantine** on the repository is no longer pinned.
</Warning>
...
Check **Settings → Quarantining** on the collection. Collections created since September 2026 start with quarantining enabled; older ones start with it off, and were not changed. Auto-quarantine is off either way, so until you turn it on only tests you have set an **Always Quarantine** override on are quarantined. Auto-quarantine covers tests a monitor has flagged **flaky** — a **broken** test is never a candidate.
</Accordion>
