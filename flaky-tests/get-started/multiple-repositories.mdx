...
## Collections, repositories, and variants

Three things group tests, and they answer different questions:
...
A collection can span repositories, and a repository's tests can be split across collections — which one an upload lands in is decided by the collection ID you pass.
...
<Frame>
  ![The Fork PR Uploads setting in a repository's Flaky Tests settings, showing the enabled toggle and the public repo identifier](/assets/flaky-tests/get-started/fork-pr-uploads-setting.png)
</Frame>
