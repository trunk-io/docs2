...
og:title: "Using Atlassian Bamboo with Trunk Flaky Tests"
---
...
Add an `Upload Test Results` step after running tests in each of your Bamboo jobs that run tests. This should be minimally all jobs that run on pull requests, as well as from jobs that run on your [stable branches](../../detection#branch-aware-detection), for example, `main`, `master`, or `develop`.
...
variables:
  TRUNK_ORG_URL_SLUG: <YOUR_TRUNK_ORG_URL_SLUG>
  TRUNK_API_TOKEN: <YOUR_TRUNK_API_TOKEN>
```
```yaml Bazel
version: 2
plan:
