...
jobs:
  test:
    ...
    steps:
      - name: Run Tests
        run: swift test --parallel --xunit-output junit.xml
      ...
      - name: Upload Test Results to Trunk.io
        ...
        uses: trunk-io/analytics-uploader@v2
        with:
          # Files, not globs. One run writes both: swift-testing results to
          # junit-swift-testing.xml and XCTest results to junit.xml.
