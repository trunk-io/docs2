  ...
  test:
    ...
    steps:
      ...
      - name: Upload Test Results to Trunk.io
        ...
        uses: trunk-io/analytics-uploader@v2
        with:
          # One run writes both: swift-testing results to junit-swift-testing.xml
          # and XCTest results to junit.xml, so one glob covers the pair.
          swift-test-xunit-paths: junit*.xml
          org-slug: <TRUNK_ORG_URL_SLUG>
          token: ${{ secrets.TRUNK_API_TOKEN }}
```
```yaml RSpec plugin
