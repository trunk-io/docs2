...
og:title: "Using GitLab with Trunk Flaky Tests"
---
...
Store the Trunk slug and API token obtained in the previous step in your GitLab CI/CD pipelines as new [GitLab Variables](https://docs.gitlab.com/ee/ci/variables/index.html#for-a-project) named `TRUNK_ORG_URL_SLUG` and `TRUNK_TOKEN` respectively.
...
upload_test_results: # This job uploads tests results run in the last stage to Trunk.io
  ...
  script:
    ...
    - ./trunk-analytics-cli upload --junit-paths "<XML_GLOB_PATH>" --org-url-slug <TRUNK_ORG_URL_SLUG> --token $TRUNK_TOKEN
```
```yaml Bazel
image: node:latest
...
upload_test_results: # This job uploads tests results run in the last stage to Trunk.io
  ...
  script:
    - curl -fL --retry 3 "https://github.com/trunk-io/analytics-cli/releases/latest/download/trunk-analytics-cli-x86_64-unknown-linux.tar.gz" | tar -xz && chmod +x trunk-analytics-cli
    - ./trunk-analytics-cli upload --bazel-bep-path <BEP_JSON_PATH> --org-url-slug <TRUNK_ORG_URL_SLUG> --token $TRUNK_TOKEN
```
```yaml XCode
image: node:latest
...
upload_test_results: # This job uploads tests results run in the last stage to Trunk.io
  ...
  script:
    - curl -fL --retry 3 "https://github.com/trunk-io/analytics-cli/releases/latest/download/trunk-analytics-cli-x86_64-unknown-linux.tar.gz" | tar -xz && chmod +x trunk-analytics-cli
    - ./trunk-analytics-cli upload --xcresult-path <XCRESULT_PATH> --org-url-slug <TRUNK_ORG_URL_SLUG> --token $TRUNK_TOKEN
```
```yaml RSpec plugin
image: node:latest
...
unit_test_job:   # This job runs the tests and uploads the results to Trunk.io
  stage: test    
  script:
    - TRUNK_ORG_URL_SLUG=$TRUNK_ORG_URL_SLUG TRUNK_API_TOKEN=$TRUNK_TOKEN bundle exec rspec
```
</CodeGroup>
...
If a recent run isn't showing up, check your CI logs for upload errors and confirm your `TRUNK_TOKEN` and `TRUNK_ORG_URL_SLUG` secrets are set on the project running the pipeline.
