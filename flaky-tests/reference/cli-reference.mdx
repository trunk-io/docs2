...
<Info>
The uploaded tests are processed by Trunk periodically, not in real-time. Wait for at least an hour after the initial upload before they’re displayed in the [Uploads tab](/flaky-tests/get-started#step-3-verify-integration). Multiple uploads are required before a test can be accurately detected as flaky.
</Info>

Trunk accepts uploads from three main report sources, [JUnit XML](https://github.com/testmoapp/junitxml), [Bazel Event Protocol JSONs](https://bazel.build/remote/bep#consuming-bep-text-json), and Swift (both XCode XCResult paths and the XML written by `swift test --xunit-output`). You can upload each of these test report sources using the `./trunk-analytics-cli upload` command like this:
...
<Tab title="Swift">
Trunk can accept the XML written by `swift test --xunit-output` through the `--swift-test-xunit-paths` argument:

```
./trunk-analytics-cli upload --swift-test-xunit-paths "junit-swift-testing.xml,junit.xml" \
   --org-url-slug <TRUNK_ORG_URL_SLUG> \
   --token $TRUNK_API_TOKEN
```

This XML carries no file path for any test case, so Trunk resolves each test to the file it is declared in by reading your checkout. See [Swift Testing](/flaky-tests/get-started/frameworks/swift-testing) for what a single `swift test` run writes and what the resolution requires.
</Tab>
</Tabs>
...
The collection ID is eight alphanumeric characters, and you copy it from the collection in the Trunk app. `--org-url-slug` is still required — the collection ID is an addition to it, not a replacement.

You can set the ID as the `TRUNK_TEST_COLLECTION_ID` environment variable instead, and the [uploader action](/flaky-tests/get-started/ci-providers/github-actions) takes it as the `test-collection-id` input.

<Note>
Each invocation uploads to a single collection. To send the same results to more than one collection, run the upload once per collection.
</Note>

When a collection ID is passed, the links printed at the end of a run point at the collection rather than the repository. Pass `--hide-test-collection-links` to keep the repository links.
...
```
./trunk-analytics-cli test --swift-test-xunit-paths "junit-swift-testing.xml,junit.xml" \
   --org-url-slug <TRUNK_ORG_URL_SLUG> \
   --token $TRUNK_API_TOKEN \
   <YOUR_TEST_COMMAND>
```
...
| `trunk-analytics-cli test <COMMAND>` | Runs tests using the provided command, uploads results, checks whether the failures are [quarantined](../quarantining/#updates-in-ci) tests, and correct the exit code based on that. |
...
| `--xcresult-path <XCRESULT_PATH>` | Path to a `.xcresult` directory, which contains test reports from `xcodebuild`. macOS only. |
| `--swift-test-xunit-paths <PATHS>` | Comma-separated list of glob patterns to files written by `swift test --xunit-output`, resolved against the repository root like `--junit-paths`. This XML carries no file path, so each test is attributed to the file it is declared in, resolved from your checkout. Can also be set with the `TRUNK_SWIFT_TEST_XUNIT_PATHS` environment variable. Requires CLI 0.15.5 or newer. See [Swift Testing](/flaky-tests/get-started/frameworks/swift-testing). |
| `--use-experimental-xcresult-test-locations` | **Beta.** Take each `.xcresult` test's file from where it is declared rather than from the failure that surfaced it, which also gives passing tests a file. macOS only, and requires CLI 0.15.5 or newer. See [XCTest](/flaky-tests/get-started/frameworks/xctest#attributing-tests-to-their-declaring-file). |
...
| `--test-collection-id <COLLECTION_ID>` | Upload to a [test collection](/flaky-tests/test-collections), in addition to (not instead of) `--org-url-slug`. Can also be set with the `TRUNK_TEST_COLLECTION_ID` environment variable. **Optional**. |
