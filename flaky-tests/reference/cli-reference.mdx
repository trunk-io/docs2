...
| `--swift-test-xunit-paths <PATHS>` | Comma-separated list of files written by `swift test --xunit-output`. Takes files, not globs. This XML carries no file path, so each test is attributed to the file it is declared in, resolved from your checkout. Can also be set with the `TRUNK_SWIFT_TEST_XUNIT_PATHS` environment variable. Requires CLI 0.15.5 or newer. See [Swift Testing](/flaky-tests/get-started/frameworks/swift-testing). |
