...
| Flaky Tests | Number of test spans (test case results uploaded) | 5 committers and 5M test spans per month |
...
| Flaky Tests | Number of test spans (test case results uploaded) | 1 million test spans per seat per month. $3 for each additional 1 million test spans. |
...
At the end of every billing cycle, Trunk calculates what the next invoice should be. Trunk determines feature usage and the number of seats used over the [free tier usage limits](./billing#free-plan) . See our section on [calculating user counts](./billing#calculating-user-counts) to determine how much we charge per feature usage.
...
You can cancel an active Trunk subscription by navigating to **Settings** → **Billing** and clicking the **Cancel Subscription** button. Your plan will transition back into the [free tier](./billing#free-plan); if you want to re-enable this plan, please contact us at [sales@trunk.io](mailto:sales@trunk.io).
