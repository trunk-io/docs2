...
| **Published docs** | Existing documentation pages, hierarchy from `docs.json`                                 | `claude.ai trunk docs` |
...
gh pr list --repo trunk-io/docs2 --state open --json number,title,headRefName | jq '.[] | select(.headRefName | contains("indefinite-monitor-muting"))'

# If either returns results, clean up:
# gh pr close <number> --repo trunk-io/docs2 --delete-branch
...
> "Both. New pages get created and added to docs.json. Updates edit existing files in place."
...
```bash
gh pr close <PR_NUMBER> --repo trunk-io/docs2 --delete-branch
git checkout main
