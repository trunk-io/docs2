...
description: "Combine optimistic merging and pending failure depth to prevent flaky test failures from blocking the merge queue."
og:title: "Anti-flake protection in Trunk Merge Queue"
---
## What it is
...
{/* Animated SVG prototype: stands in for the video while it renders, and reads correctly when motion is reduced. */}
<Frame caption="SVG prototype — anti-flake protection with optimistic merging + pending failure depth">
<img src="/assets/merge-queue/optimizations/anti-flake-protection-prototype.svg" alt="A right-anchored predictive merge queue: a at the front, then b, then c at the back. b fails a flaky test but is held by pending failure depth instead of being evicted. c then passes with b's changes included, proving b's failure was flaky, so optimistic merging lands a, b, and c together." />
</Frame>

<Frame caption="Anti-flake protection with optimistic merging + pending failure depth">
...
<table><thead><tr><th width="331">what's happening?</th><th>queue</th></tr></thead><tbody><tr><td><strong>A</strong>, <strong>B</strong>, <strong>C</strong> begin predictive testing</td><td><code>main</code> &#x3C;- <strong>A</strong> &#x3C;- <strong>B</strong>+a &#x3C;- <strong>C</strong>+ba</td></tr><tr><td><strong>B</strong> fails testing</td><td><code>main</code> &#x3C;- <strong>A</strong> &#x3C;- <span style={{color: 'red'}}><strong style={{color: 'red'}}>B</strong>+a</span> &#x3C;- <strong>C</strong>+ba</td></tr><tr><td>predictive failure depth keeps <strong>B</strong> from being evicted while <strong>C</strong> tests</td><td><code>main</code> &#x3C;- <strong>A</strong> &#x3C;- <span style={{color: 'red'}}><strong style={{color: 'red'}}>B</strong>+a</span> (hold) &#x3C;- <strong>C</strong>+ba</td></tr><tr><td><strong>C</strong> passes</td><td><code>main</code> &#x3C;- <strong>A</strong> &#x3C;- <span style={{color: 'red'}}><strong style={{color: 'red'}}>B</strong>+a</span> &#x3C;- <span style={{color: 'green'}}><strong style={{color: 'green'}}>C</strong>+ba</span></td></tr><tr><td>optimistic merging allows <strong>A</strong>, <strong>B</strong>, <strong>C</strong> to merge</td><td><code>merge</code> <strong>A B C</strong></td></tr></tbody></table>
