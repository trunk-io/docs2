...
<Frame>
![](/assets/merge-queue/optimizations/1768426960-batching-settings.avif)
</Frame>
...
{/* SVG prototype below shows the CORE batching idea as a without-vs-with speed comparison (6 individual test runs vs 2 batched runs). The existing video also layers in pending failure depth + optimistic merging. Timing lives in the .svg. */}
<Frame caption="SVG prototype — fewer test runs with batching">
<img src="/assets/merge-queue/optimizations/batching-prototype.svg" alt="A comparison of testing six pull requests without versus with batching. Without batching there are six test runs, one PR at a time, so they reach main slowly. With batching the PRs are grouped into two batches, ABC and DEF, each tested in a single run, so the same six PRs reach main much sooner. Main is on the right and PRs flow right to left." />
</Frame>
