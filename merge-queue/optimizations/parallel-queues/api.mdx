...
<Info>
The documented request body limit is 20 MiB (see the [API reference](../../reference/merge#request-format)).
Engineering has cited ~10 MB in customer support threads. If you are anywhere close
to either number, confirm the current limit with Trunk support before relying on
headroom. If your target list exceeds the limit, send `"ALL"` as the
`impactedTargets` value instead of a full list.
</Info>
