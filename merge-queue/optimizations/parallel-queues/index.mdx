...
<img src="/assets/merge-queue/optimizations/parallel-queues/dynamic-parallel-queues.svg" alt="Three dynamic parallel queues split by impacted target. A frontend lane runs PR A then PR C, a backend lane runs PR B then PR C, and a docs lane runs PR D. PR C appears in both the frontend and backend lanes because it impacts both targets." />
</Frame>
