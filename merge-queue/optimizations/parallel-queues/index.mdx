...
{/* Animated SVG prototype of parallel queues, sits above the source video */}
<Frame caption="SVG prototype — track impacted code of each pull request to create dynamic queues">
<img src="/assets/merge-queue/optimizations/parallel-queues/parallel-queues-prototype.svg" alt="Two stacked panels contrast a single queue with parallel queues. A single lane runs PRs A, B, C, D in one slow chain, while three lanes split by impacted target (frontend, backend, docs) run concurrently and finish sooner." />
</Frame>
...
<img src="/assets/merge-queue/optimizations/parallel-queues/file.excalidraw_(6).svg" alt="Three Dynamic Parallel Queues" class="gitbook-drawing"/>
</Frame>
