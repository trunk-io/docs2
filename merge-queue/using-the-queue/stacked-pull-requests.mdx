...
| `--priority` | `-p` | [Priority level](../optimizations/priority-merging) for the stacked PR in the merge queue. |
| `--no-batch` |  | Prevent the stacked PR from being [batched](../optimizations/batching) with other items in the queue. |
