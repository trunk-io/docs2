---
title: "FAQ"
description: "Answers to common questions about Trunk Merge Queue behavior and the PR lifecycle."
---
...
<AccordionGroup>
  ...
  <Accordion title="Why did my pull request fail with a GitHub API rate limit error?">
    Trunk Merge Queue calls the GitHub API on your behalf during the merge process. If your organization's GitHub API quota is exhausted, Trunk cannot complete the merge and removes the pull request from the queue with a `GitHub API rate limit exceeded` failure reason.

    This is a GitHub-side quota, shared across all applications authenticated to your organization. To resolve this:

    - Wait for GitHub's rate limit window to reset (typically one hour), then re-queue the pull request.
    - Reduce the number of automated tools or GitHub Apps that make frequent API calls against your organization.
    - Check your current rate limit usage in GitHub under **Settings** > **Developer settings** > **GitHub Apps**, or via the [GitHub rate limit API](https://docs.github.com/en/rest/rate-limit/rate-limit).

    If you see this error regularly, contact [support@trunk.io](mailto:support@trunk.io) to discuss your usage patterns.
  </Accordion>
