...
`since` and `until` are both optional and bound the window in which a PR concluded (merged, failed, or cancelled). Send either one on its own and the other end of the range fills in a default — `since` defaults to the beginning of time, `until` defaults to now. Omit both and results are not filtered by conclusion time at all.
