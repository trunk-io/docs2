...
og:title: "Trunk Merge Queue settings and configuration"
---
...
<Frame>
![](/assets/merge-queue/administration/SCR-20260202-obcl.png)
</Frame>
...
<img src="/assets/merge-queue/administration/testing-concurrency.svg" alt="A queue of six PRs where only two test at a time inside a concurrency window; as the front pair finishes, the window slides to the next pair." />
</Frame>
...
<Frame>
![](/assets/merge-queue/administration/merge-github-comment_(1).png)
</Frame>
...
Use GitHub PR labels to enqueue and dequeue pull requests, as an alternative to [GitHub commands](./advanced-settings#github-commands) or the merge checkbox. Adding the configured label to a PR submits it to the queue; removing the label cancels it.
...
<Info>
**Note:** Once a PR has been given a state label, Trunk continues to manage that PR's labels even if this setting is later disabled. Disabling State Labels stops new PRs from being labeled, but PRs already labeled keep having their labels updated.
</Info>
