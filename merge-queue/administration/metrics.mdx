...
Each histogram also has companion percentile gauges — `_p50`, `_p75`, `_p90`, `_p95`, and `_p99` — carrying the same labels as the histogram they belong to. These are computed by Trunk over the same rolling window, so you can read a quantile directly instead of interpolating one out of the buckets with `histogram_quantile()`.
...
| `state` | Meaning |
| --- | --- |
| `not_ready` | Waiting to be admitted to the queue, on prerequisites such as required reviews, status checks, or GitHub mergeability |
| `pending` | In the queue, waiting for a testing slot |
| `testing` | Running the required tests |
| `tests_passed` | Tests passed, waiting on the PRs ahead of it to merge first |
| `pending_failure` | Held behind a PR that failed, under [pending failure depth](../optimizations/pending-failure-depth) |
| `merged`, `failed`, `cancelled` | Terminal states — see the caveat below |

The label is not a fixed list. It reports the state your queue actually recorded, so a state added later shows up as a new label value rather than going missing.
...
<Warning>
**Terminal states measure something different.** `merged`, `failed`, and `cancelled` are only ever left when the same PR is re-submitted to the queue, so those series measure time-until-resubmit, which commonly runs to days and lands in the `+Inf` bucket. Filter them out (for example `state!~"merged|failed|cancelled"`) before aggregating across states, or they will drag your quantiles out.
</Warning>

`state="pending"` and `state="testing"` report the same durations as `mq_pr_wait_duration_1h_seconds` and `mq_pr_test_duration_1h_seconds`. Those two metrics are unchanged and are not going away, so existing dashboards and alerts keep working as they are.
...
# Where is the time going? Mean seconds per state, terminal states excluded
sum by (state) (mq_pr_state_duration_1h_seconds_sum{state!~"merged|failed|cancelled"})
  /
sum by (state) (mq_pr_state_duration_1h_seconds_count{state!~"merged|failed|cancelled"})

# Alert when PRs are queueing up behind a failure
mq_pr_state_duration_1h_seconds_p95{state="pending_failure"} > 1800

# P95 for one state, computed from the buckets
histogram_quantile(0.95, sum(mq_pr_state_duration_1h_seconds_bucket{state="testing"}) by (le))
```
