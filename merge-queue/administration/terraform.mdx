...
og:title: "Manage Trunk Merge Queue with Terraform"
---
...
| `bisection_concurrency` | integer | Concurrency for bisection testing during batch failure isolation. See [Bisection Testing Concurrency](./advanced-settings#bisection-testing-concurrency). |
...
<Frame caption="Banner for Merge Queue settings showing that the Merge Queue is managed by Terraform.">
![](/assets/merge-queue/administration/terraform-in-sync.png)
</Frame>
...
<Frame caption="Banner for Merge Queue settings showing that the Merge Queue is managed by Terraform but has since drifted due to settings being changed manually in the UI.">
![](/assets/merge-queue/administration/terraform-drift-mergequeue.png)
</Frame>
