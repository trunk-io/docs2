...
On pull requests managed by Trunk, the extension can hide parts of GitHub's native UI that its own panel makes redundant. Two toggles hide status rows in GitHub's **merge box** (the panel at the bottom of a pull request that shows whether it can be merged); a third hides Trunk's own merge-queue bot comments. Settings are saved per browser profile. To configure them, click the Trunk toolbar icon, open **Settings**, and find the **Hide rows** card.
...
| **Branch out of date** | GitHub's informational Conflicts row when there are no actionable conflicts. Rows with "This branch has conflicts that must be resolved" or a **Resolve conflicts** control are always left visible. | Off |
...
<AccordionGroup>
  <Accordion title="Do I need to set up anything besides the extension?" defaultOpen>
