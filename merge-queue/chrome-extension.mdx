...
On pull requests managed by Trunk, the extension automatically hides Trunk bot comments that prompt you to submit a PR to the merge queue. These comments (from `trunk-io` and `trunk-staging-io` bot accounts) contain text like "To merge this pull request, check the box" or include a `/merge-queue/` link.
...
<AccordionGroup>
<Accordion defaultOpen title="Do I need to setup anything besides the extension?">
...
<Accordion defaultOpen title="Why don't I see the overlay on a PR?">
...
<Accordion defaultOpen title="Does the extension work in other Chromium browsers?">
...
<Accordion defaultOpen title="How does the extension differ from the `/trunk merge` comment?">
...
<Accordion defaultOpen title="How do I enable Celebration Mode?">
