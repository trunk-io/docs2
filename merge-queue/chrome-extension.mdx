...
<AccordionGroup>
  ...
  <Accordion title="Does the extension work in Firefox?" defaultOpen>
    Yes. A Firefox build is available and has been validated against the Mozilla Add-ons (AMO) requirements. The AMO listing is coming soon. In the meantime, the extension can be loaded as a temporary add-on in Firefox Developer Edition or Nightly using the production build.
  </Accordion>
