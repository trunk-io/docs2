...
Webhook payloads currently **do not include an event timestamp**. The practical workaround is to treat the time your endpoint receives the webhook as the event time. If you need precise event ordering, persist the receive-time on your side as soon as the payload arrives.
