...
On Buildkite it is a plugin instead, which decides at pipeline-upload time so a skipped
step never acquires an agent. See [Dynamic CI for Buildkite](/dynamic-ci/buildkite).
...
<Note>
  Running Buildkite? Follow [Dynamic CI for Buildkite](/dynamic-ci/buildkite)
  instead — the setup, the step identity and the rollout all differ.
</Note>
